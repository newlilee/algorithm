...
/**
 * 39. Combination Sum
 * 给你一个无重复元素 的整数数组candidates 和一个目标整数target，找出candidates中可以使数字和为目标数target的 所有不同组合，并以列表形式返回。你可以按任意顺序返回这些组合。
 * <p>
...
public class CombinationSum {
	...
	private static void backtracking(List<List<Integer>> result, int[] array, List<Integer> subList, int remain, int nextIdx) {
		if (remain < 0) {
			return;
		}
		if (remain == 0) {
